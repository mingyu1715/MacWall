...
- [x] `NativeRuntimePlaybackControlUpdate` Codable round-trip test를 추가한다.
- [x] status의 suspended 진단 필드가 이전 JSON과 호환되는지 테스트한다.
- [x] `playback-control.json` atomic read/write와 cleanup 테스트를 추가한다.
- [x] active/candidate/stale generation 적용 결정을 순수 policy로 테스트한다.
- [x] `NativeRuntimeSessionState.stop()`이 active/candidate 상태를 모두 지우는 테스트를 추가한다.
- [x] focused test가 RED인 것을 확인한다.
- [x] 최소 구현으로 GREEN을 만든다.
...
- [x] fake scheduler를 사용해 visibility 200ms debounce 테스트를 작성한다.
- [x] 빠른 covered/visible 변화가 마지막 상태 하나만 publish하는지 테스트한다.
- [x] sleep 즉시 suspend와 wake 500ms 재평가를 테스트한다.
- [x] option off가 즉시 resume하고 이후 coverage를 무시하는지 테스트한다.
- [x] Native inactive 및 Legacy active일 때 control을 발행하지 않는지 테스트한다.
- [x] `AppViewModel`이 Native Play 성공/Stop 성공에만 controller active 상태를 바꾸고 failing transition에서는 기존 상태를 유지하는지 테스트한다.
- [x] RED 확인 후 visibility monitor, notification, scheduler를 주입 가능한 controller로 구현한다.
...
- [x] backend가 active generation을 대상으로 control update를 쓰고 Darwin notification을 보내는 테스트를 추가한다.
- [x] coordinator가 최신 desired suspension을 기억하는 테스트를 추가한다.
- [x] active A가 있으면 B 준비 중에도 A에 즉시 control을 보내는 테스트를 추가한다.
- [x] B 성공 후 최신 state를 B generation에 다시 publish하는 테스트를 추가한다.
- [x] failing B가 A의 active receipt와 control 상태를 보존하는 테스트를 추가한다.
- [x] Legacy/no-active 경로가 native control file을 쓰지 않는지 테스트한다.
- [x] RED 확인 후 최소 API와 구현을 추가한다.
...
- [x] source guard에 playback control polling, generation 검증, reversible bridge API 조건을 추가한다.
- [x] `NativeVideoFrameBridge.setPlaybackSuspended(_:)`를 구현한다.
- [x] suspend가 media-data request와 clock만 멈추고 reader/pending sample/last frame을 보존하게 한다.
- [x] resume이 새 pump generation에서 request를 다시 시작하게 한다.
- [x] active control은 즉시 bridge 전체에 적용한다.
- [x] candidate control은 state만 저장하고 first-frame commit 이후 적용한다.
- [x] extension startup과 context rebuild가 persisted Play/control 순서로 복원되는지 source guard로 확인한다.
...
- [x] generation별 first-failure retry, second-failure exhaust, new-generation reset을 순수 policy test로 작성한다.
- [x] renderer callback에 generation, instance ID, context identity가 전달되는 source guard를 추가한다.
- [x] candidate failure와 active failure가 분리되는 guard를 추가한다.
- [x] 첫 active failure가 old set을 유지한 채 persisted Play로 replacement candidate를 시작하게 한다.
- [x] replacement commit 후 old set cleanup과 suspension state 적용을 확인한다.
- [x] 같은 generation의 두 번째 실패가 모든 active bridge를 freeze하고 failed status를 기록하게 한다.
- [x] stale callback을 무시한다.
...
- [x] Stop이 extension session의 active/candidate instance, generation, suspension state를 clear하는 guard를 추가한다.
- [x] Stop ACK 전에 generation을 삭제하지 않는 backend test를 추가한다.
- [x] Stop ACK 후 all generation directories와 transient display/control files를 정리하는 store test를 추가한다.
- [x] runtime root 및 QA transport 설정이 보존되는지 테스트한다.
- [x] cleanup 실패가 Stop ACK 결과를 뒤집지 않고 진단 가능한지 구현한다.
...
- [x] Native Video의 자동 일시정지 조건과 마지막 frame 유지 정책을 README KO/EN에 반영한다.
- [x] 구현 결과와 복구 제한을 implemented 문서에 정리한다.
- [x] KST timestamp와 focused/전체 test 결과를 development log에 기록한다.
- [x] 활성 설계/계획 링크를 문서 index에 반영한다.
- [x] focused tests, 전체 `swift test`, source guard를 실행한다.
- [x] 필요할 때만 AdHocQA compile gate를 실행한다. GUI는 실행하지 않는다.
- [x] diff와 금지 범위를 최종 검토한다.
- [x] 변경을 로컬 commit으로 남긴다.
