...
private let macWallChoiceIdentifier = "macwall-native-wallpaper-choice"

func makeMacWallSettingsViewModelsXPC() -> AnyObject? {
    let bundleID = Bundle.main.bundleIdentifier ?? "io.github.mingyu1715.MacWall.NativeWallpaper"
    guard let thumbnailURL = makeMacWallThumbnailURL() else {
    ...
    let choiceID = StubChoiceID(
        id: macWallChoiceIdentifier,
        descriptor: StubChoiceIDDescriptor(
            provider: StubChoiceProviderID(rawValue: bundleID),
            identifier: macWallChoiceIdentifier,
            files: [thumbnailURL],
            configuration: Data(macWallChoiceIdentifier.utf8)
        )
    )
    ...
    let choiceDescriptor = StubChoiceDescriptor(
        ...
        identifier: macWallChoiceIdentifier,
        name: "MacWall",
        localizedDescription: "MacWall native wallpaper runtime",
    ...
    let group = StubSettingsGroup(
        id: StubGroupID(id: "macwall-native-wallpaper"),
        items: [item],
        localizedName: "MacWall",
        disposability: .none,
        sortOrder: -100,
...
private func makeMacWallThumbnailURL() -> URL? {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("MacWallNativeWallpaper", isDirectory: true)
