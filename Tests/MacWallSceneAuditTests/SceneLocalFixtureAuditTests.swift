...
final class SceneLocalFixtureAuditTests: XCTestCase {
    func testLocalSceneFixturesMatchTrackedAggregateCatalog() throws {
        ...
        for fixture in availableFixtures {
            let packageURL = packageURL(for: fixture)
            ...
            XCTAssertEqual(
                counted(
                    report.textures.compactMap(\.formatRawValue)
                ),
            ...
            XCTAssertEqual(
                counted(
                    report.textures.compactMap(\.flagsRawValue)
                ),
            ...
            let features = Dictionary(
                uniqueKeysWithValues: report.features.map {
            ...
            XCTAssertFalse(
                recording.readRanges.contains(
                    0..<recording.byteCount
                ),
                fixture.workshopID
            )
            ...
            XCTAssertFalse(
                report.diagnostics.contains {
                    $0.code == "package.invalid-index"
                        || $0.code == "texture.invalid-metadata"
                },
                fixture.workshopID
            )
            XCTAssertTrue(
                report.textures.allSatisfy {
...
private func counted<T: Hashable>(
    _ values: [T]
) -> [String: Int] {
