...
struct SceneDXTDecoder: Sendable {
    ...
    func decode(_ data: Data, width: Int, height: Int) throws -> Data {
        guard width > 0, height > 0 else {
            throw SceneFormatError.invalidRange("texture-dimensions")
        }
        let blocksWide = (width + 3) / 4
        let blocksHigh = (height + 3) / 4
        let blockSize = format == .dxt1 ? 8 : 16
        let (blockCount, countOverflow) =
            blocksWide.multipliedReportingOverflow(by: blocksHigh)
        let (expectedBytes, byteOverflow) =
            blockCount.multipliedReportingOverflow(by: blockSize)
        guard !countOverflow, !byteOverflow, data.count >= expectedBytes else {
            throw SceneFormatError.truncated
        }

        let (pixelCount, pixelOverflow) =
            width.multipliedReportingOverflow(by: height)
        let (rgbaByteCount, rgbaOverflow) =
            pixelCount.multipliedReportingOverflow(by: 4)
        guard !pixelOverflow, !rgbaOverflow else {
    ...
    private func decodeColorBlock(
        ...
        let color0 = try readUInt16(data, offset: offset)
        let color1 = try readUInt16(data, offset: offset + 2)
        let lookup = try readUInt32(data, offset: offset + 4)
        let colors = colorPalette(
            color0: color0,
            color1: color1,
            allowOneBitAlpha: allowOneBitAlpha
        )
        for localY in 0..<4 {
            ...
            for localX in 0..<4 {
                ...
                let paletteIndex = Int(
                    (lookup >> UInt32(localIndex * 2)) & 0x03
                )
    ...
    private func decodeDXT5Alpha(
        ...
        var bits: UInt64 = 0
        for index in 0..<6 {
            bits |= UInt64(data[offset + 2 + index])
                << UInt64(index * 8)
        }
        return (0..<16).map { index in
            let paletteIndex = Int(
                (bits >> UInt64(index * 3)) & 0x07
            )
            return palette[paletteIndex]
        }
    }

    private func alphaPalette(
        alpha0: UInt8,
        alpha1: UInt8
    ) -> [UInt8] {
        if alpha0 > alpha1 {
    ...
    private func weightedAlpha(
        ...
        UInt8(
            (Int(first) * firstWeight + Int(second) * secondWeight)
                / divisor
        )
    }
