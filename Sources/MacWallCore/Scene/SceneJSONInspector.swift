...
struct SceneJSONInspector: Sendable {
    ...
    func inspect(
        ...
        let packagePaths = Set(package.entries.map(\.path))
        ...
        evidence.featureCounts[.unresolvedAsset] = evidence.dependencies
            .filter { $0.resolution == .unresolved }
            .count
        return evidence
    }

    private func canvas(from scene: [String: Any]) -> SceneAuditCanvas? {
    ...
    private func inspectObjects(
        ...
        for value in objects {
            ...
            evidence.objectKinds[classification.name, default: 0] += 1
            evidence.featureCounts[classification.feature, default: 0] += 1

            if object["parent"] != nil {
                evidence.featureCounts[.parentGraph, default: 0] += 1
            }
            if object["instance"] != nil || object["instanceoverride"] != nil {
                evidence.featureCounts[.instance, default: 0] += 1
            }
            if containsAnimation(object) {
                evidence.featureCounts[.animatedProperty, default: 0] += 1
            }
        }
    }
    ...
    private func containsAnimation(_ value: Any) -> Bool {
        if let dictionary = value as? [String: Any] {
            ...
            return dictionary.values.contains(where: containsAnimation)
        }
        ...
        if let dictionary = value as? [String: Any] {
            for key in dictionary.keys.sorted() {
                ...
                if key == "script", let script = child as? String {
                    evidence.featureCounts[.sceneScript, default: 0] += 1
                    addScriptHandlers(from: script, to: &evidence.scriptHandlers)
                }
                if Self.assetReferenceKeys.contains(key) {
                    let references = assetReferences(for: key, value: child)
                    evidence.dependencies.append(contentsOf: references.map {
                        resolve(
                            requestedPath: $0,
                            ownerPath: ownerPath,
                            key: key,
    ...
    private func assetReferences(for key: String, value: Any) -> [String] {
    ...
    private func resolve(
        ...
        if key == "texture" || key == "textures" {
            ...
            if let resolved = candidates.first(where: packagePaths.contains) {
                return SceneAuditDependency(
                    ownerPath: ownerPath,
                    key: key,
                    requestedPath: requestedPath,
        ...
        let pathExtension = URL(filePath: requestedPath).pathExtension
        if key == "shader",
    ...
    private func addScriptHandlers(
        ...
        let pattern = #"(?:export\s+)?function\s+([A-Za-z_][A-Za-z0-9_]*)\s*\("#
        ...
        for match in expression.matches(in: script, range: range) {
            guard match.numberOfRanges > 1,
                  let handlerRange = Range(match.range(at: 1), in: script) else {
                continue
            }
            counts[String(script[handlerRange]), default: 0] += 1
        }
    }
}
