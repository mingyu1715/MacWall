...
struct MacWallCtl {
    ...
    private static func printHelp() {
        print("""
        macwallctl scan <folder> [--out <index.json>]
        macwallctl import <folder> [--library <folder>]
        macwallctl import-video <video-file> [--library <folder>]
        macwallctl remove <asset-id> [--library <folder>]
        macwallctl convert <input-video> --out <output.mp4>
        macwallctl scene-info <scene.pkg>
        macwallctl scene-render-info <scene.pkg>
        macwallctl doctor
        """)
    }
}
...
private enum CLIError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidConvertUsage:
            return "usage: macwallctl convert <input-video> --out <output.mp4>"
        }
    }
}
